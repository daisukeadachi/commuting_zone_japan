...
**Denominator of the proportional flow.** The smaller of the two municipalities' counts of residents mainly working, summed over every destination the census records. That is the quantity the source paper calls the total workforce and computes as the row sum of the flow matrix. `validation/notes/denominator.md` records it, together with what widening the count to residents who work alongside housework or study does to the delineation, which is the sensitivity check that answers the question raised at the coauthor meeting.
