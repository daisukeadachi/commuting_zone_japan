...
The commuting matrix is the "mainly working" variant restricted to ordinary households, which is the only definition available for 2020. The dissimilarity between two municipalities is one minus the sum of the two directed commuting flows over the smaller of the two resident labour forces, capped so that no dissimilarity falls below 0.001. The tree is built by average linkage and cut at a fixed height. Two cutoffs are carried throughout, 0.980 from the existing Japanese pipeline and 0.977 from the source paper; the figures below are at 0.980 unless stated.
...
`validation/output/diagnostics_table.csv` carries the full table for every census year at both cutoffs. The delineation consolidates steadily, from 350 commuting zones in 1980 to 208 in 2020, while the count of single-municipality zones falls from 58 to 9 and mean zone area grows from 1,039 to 1,748 square kilometres. The largest zone holds 32 municipalities in 2020 and 5.85 million workers: the special wards together with Kawasaki, Hachioji, Machida and the inner Tama cities, the southern Saitama ring from Kawaguchi through Asaka to Fujimino, and Ichikawa and Urayasu across the river.
...
`validation/output/containment_table.csv` carries the Containment block. Mean containment across zones, the share of a municipality's residents who work inside its own commuting zone averaged over municipalities and then over zones, sits at 0.908 in 2020 and has moved little since 1980, when it was 0.917. Weighting by resident labour force lowers it to 0.874, and the share of the national labour force that lives and works in the same zone is 0.865. Work containment, the share of jobs filled from inside the zone, is 0.919 in 2020.

The zone with the lowest mean containment in 2020 is a ten-municipality cluster in north-western Chiba, Funabashi, Matsudo, Noda, Narashino, Kashiwa, Nagareyama, Yachiyo, Abiko, Kamagaya and Shiroi, at 0.499 over 911,000 workers. `lowest_containment_zone_2020_cut0.980.png` shades every municipality of that zone and of the zones adjacent to it by its own containment, over a basemap, with the zone boundaries drawn heavy and numbered and the least contained municipality named. These are commuter towns whose residents work in the Tokyo zone next to them; the proportional flow keeps them together because they resemble one another, and the denominator of the smaller labour force keeps them from joining the far larger neighbour. The same mechanism produces the low-containment cases the source paper reports for the United States.

## Similarity between delineations and the cutoff

`similarity_map_2010_2020_cut0.980.png` maps the municipality-level Jaccard similarity between the 2010 and the 2020 delineations, and `validation/output/similarity_table.csv` reports the summaries. Mean similarity for that pair is 0.844. The two pre-pandemic controls are higher: 0.880 for 2010 against 2015 and 0.862 for 2005 against 2015. Decade churn was larger further back, though, at 0.786 for 1980 against 1990 and 0.829 for 1990 against 2000, so the 2010 to 2020 figure sits inside the historical range rather than outside it.

The cutoff sweep is the central exercise, and its result is the flatness the design anticipated. `similarity_vs_cutoff_2010_2020_anchor0.980.png` shows the curve. Holding 2010 at 0.980 and varying the 2020 cutoff, mean similarity peaks at 0.856 at a cutoff of 0.978, and stays within 0.005 of that peak over the range 0.977 to 0.979, which spans 223 down to 216 commuting zones. Anchoring 2010 at 0.977 instead moves the peak to 0.975 and the plateau to 0.974 to 0.977, spanning 236 down to 223 zones. The two anchors therefore sit inside or immediately beside each other's plateau, and nothing in the objective distinguishes them. The same pattern holds for both control pairs.
...
Non-contiguity is rare in Japan. The number of zones holding a municipality that shares its zone with none of its neighbours is one in 1980, two in 1985, and zero in 2005 and 2010, rising to two in 2015 and one in 2020. The affected labour force never exceeds 0.07 percent of the national total. `noncontiguous_municipalities_2020_cut0.980.png` maps the 2020 case and `validation/output/reassignment_table.csv` lists every case with the alternative that would remove it.

The recurring cases are mountain municipalities whose road connections run away from their land neighbours. Gujō and Shirakawa in Gifu appear in every year from 1980 to 2000, Uenohara in Yamanashi in several years between 1985 and 2015, and Shimukappu in Hokkaido in 2015 and 2020. In every case the assigned zone receives far more of the municipality's commuters than any adjacent zone does, between 71 and 97 percent against no more than 10.4 percent, so reassignment would trade a contiguous map for a worse fit to the flows. That is the diagnostic's answer: the non-contiguity is a property of the commuting pattern, not a failure of the clustering, and the delineation keeps every municipality where the clustering put it.
...
`validation/output/core_table.csv` carries the Core block on two definitions. On the Urban Employment Area central cities, 69.7 percent of the 2020 commuting zones contain a core, 52.1 percent of workers commute to a core inside their own zone, and 47.8 percent of jobs are held by residents of a core inside the zone. On the mechanical rule that a municipality is a core when its densely inhabited district population reaches 10,000, which uses no commuting data at all, the same three measures read 74.5, 74.7 and 74.2 percent; at the 50,000 threshold they read 47.6, 61.6 and 60.7 percent.
