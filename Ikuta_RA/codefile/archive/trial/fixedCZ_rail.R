...
Rail.row <- Rail.row %>% 
  ...
  dplyr::mutate(start = as.integer(N05_005b),
                end = as.integer(N05_005e)) %>%
  dplyr::ungroup() %>% 
  dplyr::select(start, end) %>% 
  sf::st_transform(4612)
...
saveMap <- patchwork::wrap_plots(map_list, ncol = 3) +
  patchwork::plot_annotation(caption = paste0("黒い実線が在来線、点線が新幹線を示す。CZは", CZ_year, "年のもの。"),
                             theme = theme(plot.caption = element_text(size = 4)))
#### save ####
ggplot2::ggsave(plot =saveMap, filename = paste0("output/map_image/Railroad/fixed_CZ/1950to2015_", CZ_year, "CZandRailmap.png"), dpi = 1200)
