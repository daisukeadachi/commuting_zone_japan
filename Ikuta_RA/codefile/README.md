...
このフォルダ内のコードは、CZ・UEAについての塗り分け地図を生成するためのものです。塗分地図は以下の要件を満たしています。

1.  隣接する通勤圏/都市雇用圏に異なる色を割り当て
2.  同じ市町村から構成されるCZ・UEAは同じ色を使用(`tree-height_*.R`を除く)
3.  東京UEA及び東京都千代田区を含むCZの色を固定(`"#FFD92F"`)
...
**入力：** 
- ポリゴン集合（各ポリゴンはJISCODEでラベル付け） - グループ情報（各ポリゴンをグループに割り当て） - 色パレット
...
**制約：** 
- 隣接するポリゴンは異なる色を持つ
- 東京CZ・UEAは必ず`"#FFD92F"`で塗り、他のCZ・UEAはこの色で塗らない。
...
**目的：** 1980年及び2015年時点の関東地方のCZとUEAを並べて表示
...
- [Municipality Map Maker](http://www.tkirimura.com/mmm/)より取得した、市町村データ
...
├── 2015_original.csv
└── addCZdata/
    └── 2020_harmonized_small-0.001_tree_height-0.98.csv  # 2020年時点のデータ
```
...
- [都市雇用圏のWebサイト](https://www.csis.u-tokyo.ac.jp/UEA/uea_code.htm#/)より入手したコード表を整理
...
### 4. 鉄道時系列データ（withRail.Rで使用）

**ファイル位置：** `data/N05-23_GML/N05-23_RailroadSection2.shp`

- 新幹線・在来線を含む鉄道ネットワークのラインデータ
- [国土数値情報ダウンロードサイト](https://nlftp.mlit.go.jp/ksj/gml/datalist/KsjTmplt-N05-2024.html#/)より入手
...
- 1980年の市町村コードと2015年の市町村コードの対応表（合併対応）
- [Municipality Map Maker](http://www.tkirimura.com/mmm/)より取得
- harmonizedの地図を作成する際、都市雇用圏の市区町村コードを2015年のものと合わせるために利用
...
### ワークスペース全体の構成
- コードは必ず`Ikuta_RA`をワーキングディレクトリにして実行
