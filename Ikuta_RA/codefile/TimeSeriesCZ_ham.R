...
# This code generate harmonized CZ map (the year of municipality data is harmonized on 2015.) 
...
for (y in year){
  ...
  CZ.sf %>% 
    ...
    ggplot2::labs(title = paste(y, "近畿 CZ"))+
    ggplot2::theme(plot.title    = ggplot2::element_text(size = 5))　-> CZmap
  filepath = paste0("output/map_image/CZ/single/harmonized/whole/", y, "_whole_CZmap.png")
  ggplot2::ggsave(filename = filepath, plot = CZmap)
  rm(CZmap)
