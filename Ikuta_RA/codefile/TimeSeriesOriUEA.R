...
for (i in (1:length(path_list.McEA))){
  ...
  ggplot2::ggsave(UEAmap, filename = paste0("output/map_image/TimeSeries_UEAandCZ/Original/Whole/", year[i], "_UEAmap.png"), 
                  bg = "white", width = 5, height = 3)
  ...
  UEA.sf.enlarged %>% 
    ...
    ggplot2::theme(plot.caption = element_text(size = 3, hjust = 0)) -> UEAmap
  ggplot2::ggsave(UEAmap, filename = paste0("output/map_image/TimeSeries_UEAandCZ/Original/enlaged_MainLands/", year[i], "_UEAmap.png"), 
                  bg = "white", width = 5, height = 3)
