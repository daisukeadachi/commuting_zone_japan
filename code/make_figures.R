...
#
# Each figure is written to its own file under paper/figures and carries no
...
# The deck reads the most recent decade and the most recent census. The paper fixes its
# single-year maps at one census and sets every decade pair beside the others, so both
# sets are produced and the file names carry the year and the pair.
detail_years <- unique(c(2000L, headline_later))
decade_pairs <- appendix_pairs
sweep_pairs <- unique(c(baseline_pairs, appendix_pairs))
pair_label <- function(pair) sprintf("%d to %d", pair[1], pair[2])
...
romanized <- c(
  ...
  "12221" = "Yachiyo", "12222" = "Abiko", "12224" = "Kamagaya", "12231" = "Inzai",
  "12232" = "Shiroi", "12329" = "Sakae",
...
# same ramp over its own classes. The lowest municipality any of these maps draws sits in
# the middle thirties, so the classes start at 0.3 rather than at the floor of 0.4 the
# source paper uses, which keeps the named municipality out of an open bottom class. That
# leaves seven classes, and the ramp is taken at seven steps rather than six. The two ramps
# differ in one middle step and agree elsewhere.
similarity_breaks <- c(0, 0.03, 0.25, 0.5, 0.75, 1)
similarity_labels <- c("0", "0.03 to 0.25", "0.25 to 0.5", "0.5 to 0.75", "0.75 to 1")
containment_breaks <- c(0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1)
containment_labels <- c("0.3 to 0.4", "0.4 to 0.5", "0.5 to 0.6", "0.6 to 0.7",
...
for (pair in sweep_pairs) {
  for (anchor_value in cutoff_anchors) {
    ...
    ggsave(file.path(figure_dir, sprintf("similarity_vs_cutoff_%d_%d_anchor%s.png",
                                         pair[1], pair[2], cut_tag(anchor_value))),
           width = 8, height = 5, dpi = 300, bg = "white")
  }
}

# The same curves for the four decade pairs on one pair of axes, at the baseline anchor.
# The point of setting them side by side is that the cutoff which makes a delineation most
# stable does not move much from one decade to the next, so the value inherited from the
# United States is not being asked to do different work in different periods.
decade_curves <- sweep %>%
  filter(abs(anchor - baseline_cutoff) < 1e-9) %>%
  inner_join(bind_rows(lapply(decade_pairs, function(p) {
    tibble(earlier_year = p[1], later_year = p[2], pair = pair_label(p))
  })), by = c("earlier_year", "later_year"))
ggplot(decade_curves, aes(x = cutoff_later, y = mean_similarity,
                          group = pair, linetype = pair, colour = pair)) +
  geom_line(linewidth = 0.8) +
  geom_vline(xintercept = baseline_cutoff, linetype = "dashed", colour = "grey35") +
  scale_colour_brewer(name = "Census years compared", palette = "Dark2") +
  scale_linetype_discrete(name = "Census years compared") +
  labs(x = "Cutoff value applied to the later year", y = "Mean Jaccard similarity") +
  sweep_theme +
  theme(legend.position = "inside", legend.position.inside = c(0.17, 0.24),
        legend.background = element_blank())
ggsave(file.path(figure_dir, sprintf("similarity_vs_cutoff_decades_anchor%s.png",
                                     cut_tag(baseline_cutoff))),
       width = 8, height = 5, dpi = 300, bg = "white")
...
# Map of between-delineation similarity, the counterpart of Figure 1, for every decade
# pair. Each is written to its own file, so that a document can set them side by side or
# show one alone.
for (pair in decade_pairs) {
  for (map_cutoff in cutoff_anchors) {
    scored <- similarity_by_municipality %>%
      filter(earlier_year == pair[1], later_year == pair[2],
             abs(cutoff - map_cutoff) < 1e-9)
    mapped <- geometry %>%
      inner_join(scored, by = "code") %>%
    ...
    ggplot(mapped) +
      ...
      national_map_theme
    ggsave(file.path(figure_dir, sprintf("similarity_map_%d_%d_cut%s.png", pair[1], pair[2],
                                         cut_tag(map_cutoff))),
           width = 7.5, height = 7.5, dpi = 300, bg = "white")
  }
}

# The same four maps on one set of axes, sharing a legend. A document that sets the
# decades side by side wants this one; a document that shows a single decade wants one of
# the four above.
for (map_cutoff in cutoff_anchors) {
  faceted <- bind_rows(lapply(decade_pairs, function(pair) {
    geometry %>%
      inner_join(similarity_by_municipality %>%
                   filter(earlier_year == pair[1], later_year == pair[2],
                          abs(cutoff - map_cutoff) < 1e-9), by = "code") %>%
      mutate(pair = pair_label(pair))
  })) %>%
    mutate(pair = factor(pair, levels = vapply(decade_pairs, pair_label, character(1))),
           similarity_class = cut(similarity, breaks = similarity_breaks,
                                  labels = similarity_labels, include.lowest = TRUE))
  ggplot(faceted) +
    geom_sf(aes(fill = similarity_class, colour = similarity_class), linewidth = 0.03) +
    geom_sf(data = okinawa_frame, fill = NA, colour = "grey55", linewidth = 0.15) +
...
# One census year's maps: the detached municipalities and the commuting zone with the
# lowest mean containment. The paper reads 2000 and the deck reads the most recent
# census, so the whole block is drawn once per year in detail_years.
draw_year_maps <- function(map_year) {
  ...
  detail_selection <- function(cutoff) {
    zones <- read_csv(zone_path(map_year, cutoff),
                      col_types = cols(code = col_character(), zone = col_integer()))
    worst <- containment_by_zone %>%
      filter(year == map_year, abs(cutoff - !!cutoff) < 1e-9) %>%
  ...
  for (map_cutoff in cutoff_anchors) {
    zones <- read_csv(zone_path(map_year, map_cutoff),
                      col_types = cols(code = col_character(), zone = col_integer()))
    ...
    if (nrow(detached_here) > 0) {
      ...
      ggsave(file.path(figure_dir, sprintf("noncontiguous_municipalities_%d_cut%s.png", map_year,
                                           cut_tag(map_cutoff))),
             width = 7.5, height = 7.5, dpi = 300, bg = "white")
    }
    ...
    detail <- geometry %>%
      inner_join(zones, by = "code") %>%
      filter(code %in% detail_selection(map_cutoff)) %>%
    ...
    # The classes start above zero, so a municipality below the floor would fall out of the
    # scale and be drawn as missing. None does; this says so rather than leaving it to the eye.
    stopifnot(!any(is.na(detail$containment_class) & !is.na(detail$contained)))
    ...
    message("cutoff ", cut_label(map_cutoff), ": lowest mean containment in ", map_year,
...
for (map_year in detail_years) draw_year_maps(map_year)
