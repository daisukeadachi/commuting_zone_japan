...
#
# Writes paper/statistics/commuting_zone_crosswalk_cut<cutoff>.csv, one file per cutoff,
# and paper/statistics/commuting_zone_crosswalk_coverage.csv.
#
# The two tables meant to be handed to a user are written alongside them, under
# delineation, at the cutoff the paper uses. One carries the zones of every
...
for (cutoff in cutoff_anchors) {
  ...
  write_csv(table, file.path(delineation_dir,
                            sprintf("commuting_zone_crosswalk_cut%s.csv", cut_label(cutoff))))
...
coverage <- read_csv(file.path(delineation_dir,
                              sprintf("commuting_zone_crosswalk_cut%s.csv",
  ...
  summarise(rows = n(),
            ...
            .groups = "drop")
write_csv(coverage, file.path(delineation_dir, "commuting_zone_crosswalk_coverage.csv"))
print(as.data.frame(coverage))

# ---------------------------------------------------------------------------
# The two tables handed to a user, at the cutoff the paper uses.

provided_dir <- delineation_dir
