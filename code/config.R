...
# Sourced by every other script under code.

# Root of the folder holding the two inputs the repository does not carry, the
# commuting matrices and the municipality boundary layer. Set CZ_SHARED_ROOT to it.
shared_root <- Sys.getenv("CZ_SHARED_ROOT", "")
if (shared_root == "") {
  stop("Set CZ_SHARED_ROOT to the folder holding the commuting matrices and the ",
       "boundary layer. data/README.md says what belongs there.")
}
...
census2020_dir <- Sys.getenv("CZ_CENSUS2020_DIR", file.path(shared_root, "census2020"))
census2020_year <- 2020L

# Repository paths. Derived objects are large and are not committed.
data_dir <- "data"
derived_dir <- "derived"
output_dir <- "paper/statistics"
delineation_dir <- "delineation"
...
# The cutoff as it appears in a file name or a plot label, at three decimals.
cut_label <- function(cutoff) format(cutoff, nsmall = 3)

# The cutoff as it appears in the name of an image file. LaTeX reads everything after the
# first dot of a file name as the extension, so an image carries the cutoff without its
# decimal point.
cut_tag <- function(cutoff) gsub("[.]", "", cut_label(cutoff))
...
figure_path <- function(which = delineation) {
  ...
  file.path("paper", paste0(stem, variant_tag()))
}
