...
#
# Each fragment holds the tabular alone. The caption and the notes are written in the
# paper, beside the table.
#
# Two scopes appear. The published delineation covers every municipality the census
# reports, offshore islands included, and the diagnostic table opens with the counts on
# that scope. Everything below them replicates Fowler (2024) and is computed on the four
# main islands, the Okinawa main island and everything joined to one of them by a
# permanent road link, which is the like-for-like comparison with the source paper and
# leaves out half a percent of the labour force. Every note says which.
#
...
rows <- c(
  paste0(block_header, "{\\emph{Whole country}}", br),
  sprintf("\\quad Municipality units & %s%s", across(full, count, "municipalities"), br),
  sprintf("\\quad Commuting zones & %s%s", across(full, count, "zones"), br),
  sprintf("\\quad Offshore zones & %s%s[3pt]", across(full, count, "island_zones"), br),
  paste0(block_header, "{\\emph{Main islands}}", br),
  sprintf("\\quad Municipality units & %s%s", across(diagnostics, count, "municipalities"), br),
  sprintf("\\quad Commuting zones & %s%s", across(diagnostics, count, "commuting_zones"), br),
...
# ---------------------------------------------------------------------------
# Appendix: coverage and the three blocks over every census year, and what the
# contiguity constraint changes.
...
write_table(rows, "appendix_coverage.tex", "lrrrrrrr",
